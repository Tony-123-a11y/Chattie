...
const navLinks = [
  { href: "/dashboard/chat", label: "Recent Chats", icon: MessageSquare },
  ...
  <nav className="flex-1 overflow-y-auto px-4 space-y-0.5 pb-4">
    ...
    {navLinks.map(({ href, label, icon: Icon }) => {
      ...
      return (
        <Link
          key={href}
          href={href}
          className={`flex items-center gap-3 px-3 py-2 rounded-lg text-[15px] transition-colors ${
            active
    ...
    <div className="space-y-0.5">
      {recentChats.map((chat) => (
        <button
          key={chat}
          className="w-full text-left px-3 py-2 rounded-lg text-[13px] text-text-muted hover:bg-bg truncate transition-colors"
        >
          {chat}
        </button>
      ))}
